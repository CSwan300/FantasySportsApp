...
object App {
  ...
  // Connects to PostgreSQL database and decodes relational rows into PlayerDatabase maps
  ...
  def loadData(path: String): PlayerDatabase = {
    val db = Try(Source.fromFile(path)).toOption.map { source =>
      try {
  ...
  // Finds players whose names contain the user input string or align closely via character sets
